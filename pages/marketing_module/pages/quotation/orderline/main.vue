	...
	<div class="accounts" ref="accounts">
		...
		<div class="accounts-info" v-for="(shoppingItem, index) in shoppingItems" :key="index" v-if="shoppingItems && shoppingItems.length > 0">
			...
			<div >
				...
				<!-- <div class="accounts-info-con" v-for="(gift, giftIndex) in shoppingItem.giftList" :key="giftIndex">
				...
				if (JSON.stringify(this.addressList) === '{}') {
					...
					this.shoppingItems.forEach(item=>{
						item.goodsList.forEach(item1=>{
							shoppingGoodsIdStr.push(item1.contractId)
						})
					})
					this.orderDomainStr = [];
					let sklist = [];
					...
					this.shoppingItems.map((v, index) => {
						...
						v.packageList.map(val => {
							let list = [];
							val.contractGoodsList.forEach(item=>{
							...
							if (val.giftList) {
								...
								list = val.contractGoodsList;
							}
							let shoppingGoodsIdList = [];
							val.contractGoodsList.map(vk => {
								shoppingGoodsIdList.push(vk.contractGoodsId);
							});
							...
							this.orderDomainStr[index].packageList.push({
								...
								promotionCode: val.contractBillcode,
