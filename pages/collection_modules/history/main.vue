<template>
	<div class="myHistory">
		...
		<!-- <div class="myHistory-left" @click="()=>{$router.replace('mine')}">
        <i class="iconfont">&#xe62d;</i>
      </div> -->
			...
			<ul>
				<li v-for="(todo, todoIndex) in item.list" :key="todoIndex">
					<view>
						...
						<div class="money">
							<span style="#d66377" v-if="todo.footprintOpnum">{{ unitPrice.obpay }}{{ todo.footprintOpnum }}{{ unitPrice.mapay }}</span>
...
export default {
	...
	methods: {
		commonMounted() {
			http.post(history.queryFootprintPagePlat, { page: 1, rows: 50 }).then(res => {
