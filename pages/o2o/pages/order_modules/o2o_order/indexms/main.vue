        ...
        <li v-for="(order,index) in orderList" :key="index">
          <a>
            <div class="order-status">
              ...
              <div v-else-if="order.dataState==3">待提货</div>
            ...
            <div class="order-btn">
              ...
              <div v-if="order.dataState == 1">
                ...
                <span @click="pay(order)" :style="{borderColor:baseColor,color:baseColor}">立即支付</span>
              </div>
              ...
              <div v-else-if="order.dataState == 3">
                <div
                  ...
                  class="color-333"
                  v-if="order.contractPumode==1&&order.contractDelcode!=null"
                >提货码：{{order.contractDelcode}}</div>
                <span v-if="order.contractPumode!=1" @click="queryExpressInfo(order)">查看物流</span>
                <span v-if="order.contractPumode!=1"
...
export default {
  data() {
    return {
      ...
      //dataState//已取消（-1），初始化（0），待付款（1），待发货（2），待提货（3），成功（4）
      userImgurl:
        this.$imgDomain +
  ...
  methods: {
    ...
    orderDetail(order) {
      ...
      $router.push("o2o/pages/order_modules/o2o_order/informs", params);
      //   }
      // });
    },
    toSearch() {
