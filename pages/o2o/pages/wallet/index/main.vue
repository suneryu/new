  ...
  <div class="wallet-box">
    ...
    <div class="wallet-header">
      ...
      <div class="flex">
        <p
          class="flex-item1"
        >可提现：{{unitPrice.obpay}}{{accountObj.faccountPortion || 0}}{{unitPrice.mapay}}</p>
        <!-- <p
          class="flex-item1"
        >待入账：{{unitPrice.obpay}}{{accountObj.faccountPortion}}{{unitPrice.mapay}}</p>-->
      </div>
    </div>
    <!-- tab -->
    <div class="wallet-tab">
      ...
      <div class="tab-main">
        <ul v-show="active == 0">
          ...
          <p
            v-if="shsettlList.length == 0 && requestStaticlist"
            style="color:#999;text-align:center;padding:50rpx 0"
          >暂无返佣</p>
        </ul>
        <ul v-show="active == 1">
          <li class="flex align--center" v-for="(item,index) in accountDtList" :key="index">
            <div class="flex-item1 f-s24">
              <p>提现时间：{{item.date}}</p>
              <p>提现状态：{{item.dataState == 0 ? '待审核' : item.dataState==3?'提现完成':''}}</p>
            </div>
            <h4 class="f-s30">
          ...
          <p
            v-if="accountDtList.length == 0 && requestStatic"
            style="color:#999;text-align:center;padding:50rpx 0"
          >暂无提现</p>
        </ul>
      </div>
...
import {
  ...
  myQueryShsettlListPage,
  queryWithdrawPageByMem
...
export default {
  ...
  methods: {
    ...
    cashBtn() {
      this.$router.push("o2o/pages/wallet/cash");
    }
  }
};
</script>
