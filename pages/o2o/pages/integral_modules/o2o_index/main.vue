<template>
    <div class="integral">
      ...
      :rightIcon="rightIcon"
      />
      <div class="integral-save" @click="()=>{$router.push('my/integralRule')}">积分规则</div>
      <div class="integral-tit">
        <h2>可用积分</h2>
        <p :style="{color:'#d66377'}">{{upointsNum}}</p>
      </div>
      <div class="integral-con" v-if="items&&items.length>0">
        <ul>
          <li v-for="(item,index) in items" :key="index" >
            ...
            <div :style="{color:'#d66377'}" v-if="item.upointsListNum>0">{{item.upointsListNum}}</div>
