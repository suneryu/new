<template>
	...
	<!-- 	<div class="search-box" >
		...
		<div style="display: flex;background-color: #fff;">
			...
			<div class='constractName2' @click='parts("2")' id="constractName3" :style="{ color: fontColor3 }">线下销售合同</div>
		</div>
		<div>
			...
			<div v-for='items in contractData'>
				<div style='width: 100%;height: 150px;padding: 0 8px 0 8px;border-top: 10rpx solid #fafafa;border-bottom: 10rpx solid #fafafa;'>
					<div style='width: 100%;height: 30px;display: flex;border-bottom: 1rpx solid #E0E0E0;font-size: 11px;'>
						<div style='width:70%;line-height: 30px;'>合同编号：<span>102102102</span></div>
						<div style='width:30%;line-height: 30px;text-align: center;'>{{items.memberGname}}</div>
					</div>
					<div style='width: 100%;height: 100px;border-bottom: 1rpx solid #E0E0E0;'>
						<div style='height: 25px;width: 100%;font-size: 14px; sans-serif;font-weight: 400;font-style: normal;line-height: 25px;'>{{items.scontractName}}</div>
						<div style='height: 25px;width: 100%;font-size: 13px;line-height: 25px;'>合同描述： <span>{{items.contractRemark}}</span></div>
						<div style='height: 25px;width: 100%;display: flex;'>
							<div style='height: 25px;width: 80%;font-size: 11px;color: rgba(153, 153, 153, 0.619607843137255);line-height: 25px;'>合同金额：<span>￥{{items.goodsMoney}}</span></div>
					...
					<div style='width: 100%;height: 20px;display: flex;'>
						...
						<div class='lessMoney'  v-if='items.memberGcode == 0'>余额：{{items.memberCname}}</div>
						<div class='lessMoney' style='text-decoration: underline;width: 55%;text-align: right;' v-if='items.memberGcode == 0'>使用详情</div>
					</div>	
				</div>
			</div>
	...
	export default {
		...
		loadMore(code) {
		 ...
		 http.get(queryBuyerScontractPage, parmas)
			...
			getData(data){
				// 查询当前买家合同
				http.get(queryBuyerScontractPage, data)
					.then(res => {
						console.log("resData....",res)				
					 if(res.total>0){
						 res.rows.forEach(element => {
