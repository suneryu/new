...
export default {
	...
	onLoad(options) {
		this.goodsBeanStr = JSON.parse(options.goodsBeanStr);
		this.giftUserCode = options.giftUserCode
		// this.giftCode = options.giftCode;
		// this.giftUserId = options.giftUserId
		this.shoppingGoodsIdStr = options.shoppingGoodsIdStr.split(',');
	...
	onShow() {
		...
		http.get('/web/gt/gift/queryRelToC.json', {giftUserCode: this.giftUserCode}).then(res => {
			if (res && res.length != 0) {
				this.giftCode = res.giftCode
				this.giftUserId = res.gtGiftUserDomain.giftUserId
				this.userRelNum = res.gtGiftUserDomain.appmanageIcode || res.gtGiftUserDomain.userRelNum
				// this.goodsBeanStr = JSON.parse(this.goodsBeanStr)
				this.goodsBeanStr.forEach(item=>{
					item.giftCode = this.giftCode
				})
				this.getData()
			}
		})
	...
	methods: {
		getData(){
			...
			console.log(this.goodsBeanStr,111111111111,this.giftCode)
			if (this.pageState == 0) {
			...
			if (this.pageState == 2) {
				params = { goodsBeanStr: JSON.stringify(this.goodsBeanStr) };
		...
		calculateFreight() {
			...
			if (this.pageState == 2) {
				freightFare = {
					areaCode: this.address.areaCode,
					skuIdStr: JSON.stringify(this.goodsBeanStr)
				};
			}
		...
		savePayPrice() {
			if (JSON.stringify(this.address) === '{}') {
				...
				http.post('/web/oc/shopping/deleteShoppingGoodsBatch.json', {shoppingGoodsIdStr:JSON.stringify(this.shoppingGoodsIdStr)}).then(res => {
