					...
					<div @click="goodsDetail(item.skuCode)">
						...
						<div class="list-box">
							<div style='display: flex;'>
								<div style='display: flex;width: 50%;'><span>原价：{{ unitPrice.obpay }}{{ item.pricesetNprice }}{{ unitPrice.mapay }}</span></div>
								<div style='display: flex;width: 50%;color: #d66377'><span>采购价：{{ unitPrice.obpay }}{{ item.pricesetNprice }}{{ unitPrice.mapay }}</span></div>
							</div>
...
export default {
	...
	mounted() {
		...
		console.log("合约商城商品，this.options",this.options)
		...
		this.$qj
			...
			.then(res => {
				...
				console.log('品牌列表。',res.rows)
			});
		//查询合同商品
		// let code = this.options.scontractCode
		// this.$qj
		// 	.http(this.$qj.domain)
		// 	.get(queryScontractGoodsPageNew,this.params)
		// 	.then(res => {
		// 		this.screenList = res.rows;
		// 		console.log("aaa",res)
		// 	});
