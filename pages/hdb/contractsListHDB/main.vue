		...
		<div>
			<div v-for='(items,index) in contractData' :key="index">
				<div style='height: 180rpx;border-bottom: 1px solid #E0E0E0;margin-top: 5px;padding: 0 10px 0 10px;'>
					...
					<div class='miaoshu'>合同描述: <span>{{!qyBut?items.contractRemark:items.memberContact==null?'':items.memberContact}}</span></div>
					<div class='money'>
				...
				if (!this.qyBut) {
					...
					http.get('/web/es/searchengine/find.json', data)
						.then(res => {
							if (res.total > 0) {
								res.rows.forEach(element => {
										element.date1 = element.mschannelCode==null?null:element.mschannelCode.slice(0,10)
										element.date2 = element.mschannelName==null?null:element.mschannelName.slice(0,10)
