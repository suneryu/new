					...
					<a>
						...
						<div>
							...
							<div style="text-align:right;">
								<p style="width:140rpx;">{{ unitPrice.obpay }}{{ goods.pricesetNprice }}{{ unitPrice.mapay }}</p>
		...
		<div :style="{ display: items.dataState == 2 || items.dataState == -1 ? 'none' : 'block' }">
			<div class="orderDetail-footer">
				<div v-if="items.dataState == 1 && items.dataStatestr != 3">
					<div class="orderDetail-footer-btn" @click="cancelOrder(items)">取消订单</div>
					<div class="orderDetail-footer-btn" :style="{ borderColor: baseColor, color: baseColor }" @click="pay(items)">立即支付</div>
				</div>
				...
				<div v-else-if="items.dataState == 3">
					<div class="orderDetail-footer-btn" @click="queryExpressInfo(items)">查看物流</div>
					<div class="orderDetail-footer-btn" @click="confirmReceive(items)" :style="{ borderColor: baseColor, color: baseColor }">确认收货</div>
				</div>
	...
	methods: {
		cancelOrder(items) {
			...
			wx.showModal({
				...
				success(res) {
					if (res.confirm) {
						http.post(myOrder.cancelContractC, { contractId: items.contractId })
							.then(res => {
								if (res.success) {
									$message.alert('取消成功!');
								}
		...
		confirmReceive(items) {
			http.post(myOrder.confirmReceive, { contractBillcode: items.contractBillcode })
				.then(res => {
					if (res && res.success) {
		...
		pay(items) {
			...
			$router.push('pay/paySelect', { dataBmoney: items.dataBmoney, contractBillcode: items.contractBillcode });
		},
		evaluate(items) {
