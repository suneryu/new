<template>
	<view class="shop-cart">
		<div class="shopcart">
						...
						<li v-for="(listItem, listIndex) in listItems[index1]" :key="listIndex">
							<div v-for="(list, liIndex) in listItem.shoppingpackageList" :key="liIndex">
								<div class="memberTit" @click="titCheckBox(list, liIndex)">
									<!-- 点击全选按钮-->
									<i class="iconfont" v-if="list.titChecked == 0"
										:style="{ color: baseColor }">&#xe671;</i>
									<i class="iconfont" v-else :style="{ color: '#ededed' }">&#xe671;</i>
									{{ list.memberCname }} - <span v-if='list.goodsClass=="1"'>设备零件</span> <span
										v-if='list.goodsClass=="2"'> 印刷耗品</span> <span
										v-if='list.goodsClass=="3"'>其他</span>
								</div>
								...
								<div class="list_li">
									<div class="list_pm list_p">
										<div v-show="list.promotionCode">
											<div class="tit">
												<p :style="{ background: baseColor }" :key="pbIndex">{{ list.pbName }}
												</p>
												<span class="f-s22">
										...
										<div class="itemGoods" v-for="(item, index) in list.shoppingGoodsList"
											...
											<div class="item-container">
													...
													<div class="list-count">
														<div :style="{ color: item.dataState !== 0 ? '#ccc' : '#d66377' }"
															...
															<span style='margin-left: 5px;'
																v-if='item.goodsClass =="1" && userinfoType == "2" && checkModifyAudit == "3"'>采购价：{{ unitPrice.obpay }}{{ item.pricesetNprice1 }}{{ unitPrice.mapay }}</span>
														</div>
												...
												></view> -->
															<div class="list-add">
																...
																<div><input type="text" v-model="item.goodsCamount"
																		disabled /></div>
																<div @click.stop="add(item, index)"><i
											...
											<div
												...
												<div class="updatePm">
													...
													<span
														@click="updateAction(index, item, list.promotionName)">修改</span>
												</div>
											</div>
										</div>
									</div>
			...
			<view class="min-sale-num" v-if="listItems.length > 0 && shopSaleMinNum"
				v-bind:style="{ backgroundColor: baseColor }">{{ shopSaleMinNum }}</view>
			<div v-if="listItems.length > 0">
				<div class="shopcart-count">
					...
					<div class="del" v-else :style="{ color: baseColor, borderColor: baseColor }"
						@click="delCartShopping">删除</div>
				</div>
			</div>
			<div class="shopcart-screenLeft" v-show="isShow" @click="
				() => {
					isShow = false;
				}
			"></div>
				...
				<div class="shopcart-preferential-con">
					...
					<ol>
						<li v-for="(op, opIndex) in options" :key="opIndex">
							<h3>
								{{ op.promotionName }}
								<i class="iconfont" @click="checkPmPromotion(op)"
					...
					<div class="pre-btn" :style="{ background: baseColor }" @click="
						() => {
							isShow = false;
						}
					">
			...
			<u-popup class="quota-popup" v-model="quotaPopup" mode="center" length="90%" border-radius="20"
				:closeable="true">
	...
	export default {
		...
		methods: {
				...
				this.$qj
					...
					.then(res => {
						...
						if (res && res.rows) {
							...
							// console.log('零配件---',JSON.parse(res.rows[0].shoppingpackageList[0].areaCode))
							...
							this.typeList.map(shopCartData => {
									...
									if (v.shoppingpackageList) {
										v.shoppingpackageList.map(val => {
											...
											val.shoppingGoodsList.map(vk => {
												...
												// 第一次进来为空
												if (this.checkSkuMultipleData.length >
													0) {
								...
								shopCartData.map(v => {
									if (v.shoppingpackageList) {
										v.rowsCheck = 1;
										if (v.shoppingpackageList.filter(vm => vm.titChecked === 0)
								...
								if (v.shoppingpackageList) {
									v.shoppingpackageList.map((val, index) => {
										...
										val.shoppingGoodsList.map(vk => {
											...
											console.log(isCheckSkuMultiple,
												'---isCheckSkuMultiple')
			...
			requestCheckSaleQuota(data) {
				return this.$qj.http(this.$qj.domain).post(checkStSalequota, {
