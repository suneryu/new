<template>
  <div class="addInvoice">
    ...
    <div class="addInvoice-save" @click="()=>{isEdit=true}" v-show="show">编辑</div>
    <div class="addInvoice-info">
      <ul>
        <li>
          ...
          <input
            placeholder="请输入单位名称"
            v-model="invoice.userinvMember"
            :disabled="isEdit||!show?false:true"
          />
        </li>
        <li>
          纳税人识别码
        ...
        <li>
          ...
          <input
            placeholder="请输入开户行"
            v-model="invoice.userinvBankname"
            :disabled="isEdit||!show?false:true"
          />
        </li>
        <li>
  ...
  methods: {
    ...
    saveInfo() {
      ...
      if (this.invoice.userinvMember == "") {
        ...
        if (this.show) {
          params = {
            userinvId: this.beforeInvoice.userinvId,
            userinvCode: this.beforeInvoice.userinvCode,
            userCode: this.beforeInvoice.userCode,
            userName: this.beforeInvoice.userName,
            memberCode: this.beforeInvoice.memberCode,
            memberName: this.beforeInvoice.memberName,
            userinvSort: this.beforeInvoice.userinvSort,
            userinvUeamil: this.beforeInvoice.userinvUeamil,
            tenantCode: this.beforeInvoice.tenantCode,
            gmtCreate: this.beforeInvoice.gmtCreate,
            gmtModified: this.beforeInvoice.gmtModified,
            dataState: "0"
          };
          this.invoice = Object.assign({}, this.invoice, ...params);
          http.get(invoice.editInfo, this.invoice).then(res => {
            if (res && res.success) {
              $message.alert("编辑增票资质成功！");
              $router.replace(this.myInvoice);
            }
          });
        } else {
