<template>
    <div class="invoiceTitle">
      ...
      <div class="invoiceTitle-tit">发票类型<span :style="{color:baseColor}" @click="info">发票须知</span></div>
      <div class="invoiceTitle-con">
        <div v-for="(item,index) in items" :key="index" @click="selectInvoice(index)" :style="{background:userinvType==''+index?baseColor:'',color:userinvType==''+index?'#fff':'#999'}">{{item}}</div>
      </div>
      ...
      <div class="invoiceTitle-screenLeft" v-show="isShow" @click="(()=>{isShow=false})"></div>
      <div class="invoiceTitle-preferential" v-show="isShow">
        <div class="invoiceTitle-preferential-con">
          <p>选择已有<i class="iconfont" @click="()=>{isShow=false}">&#xe609;</i></p>
          <ul>
            <li v-for="(getinvoices,getinvoicesIndex) in getinvoicesList" :key="getinvoicesIndex" @click="selectGetInvoices(getinvoices)" >
              <h3 v-if="userinvSort=='1'"><h4>{{getinvoices.userinvMember}}<span>单位</span></h4>{{getinvoices.userinvNo}}</h3>
              <h3 v-else><h4>{{getinvoices.userinvMember}}</h4>个人</h3>
            </li>
          </ul>
          <div class="pre-btn" :style="{background:baseColor}" @click="()=>{isShow=false;$router.replace('my/addInvoiceInfo')}">新增</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    clearData(){
      ...
      this.provinceCode='11000'//省code
      this.provinceName='北京市'//省名称
      this.cityCode='110100'//市code
      this.cityName='北京市'//市名称
      this.areaCode='110101'//区code
      this.areaName='东城区'//区名称
      this.userinvMember=''
      this.userinvType='0'
      this.userinvSort='2'
      ...
      if(this.userinvMember==""||(this.userinvMember.indexOf(" ")>=0)){
        ...
        if(params.userinvType=='1'){
          ...
          if(this.userinvSort=='2'){
            ...
            http.get(invoice.writeReceipt,{paramStr:JSON.stringify(doParams)}).then(res=>{
              if(res&&res.success){
                $message.alert('提交成功！')
                $router.replace(this.myInvoice)
              }
