<template>
  <div class="footer-box">
    <div class="footer t-box">
      ...
      <div v-if="buttonshow =='1'" class="addcar t-box-flex3" @click="addCart">立即抢购</div>
...
export default {
  ...
  mounted() {},
  watch: {
    couponOnceNums(newVal, old) {
      if (newVal == 0 || newVal) {
        if (this.skuList) {
          this.actEndTimes = this.skuList.goodsOdate;
          if ($storage.get("state") == "0") {
            this.buttonshow = "0";
          } else if (this.couponOnceNumd == this.skuList.goodsProperty3 || this.couponOnceNumd > this.skuList.goodsProperty3) {
            this.buttonshow = "3";
          } else if (this.couponOnceNums == this.couponOnceNumd) {
            this.buttonshow = "3";
          } else {
            this.settimes();
          }
